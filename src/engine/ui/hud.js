...
// Returns the HUD overlay markup as a single HTML string
export function hudHTML() {
  return [
    ...
    // Top-left: title / fps / clock / discoveries
    ...
    '    ' + MONO + 'font-size:10px;color:rgba(255,255,255,.75);text-shadow:0 1px 3px rgba(0,0,0,.8);',
    '  ">N</div>',
    '</div>',

    // Bottom-left: biome / position / speed
    '<div data-stats style="',
    '  position:absolute;left:16px;bottom:16px;padding:16px;' + GLASS + 'pointer-events:none;',
    '  min-width:208px;display:flex;flex-direction:column;gap:7px;',
    '">',
    ...
    '      <span style="',
    '        width:74px;height:4px;border-radius:2px;background:rgba(255,255,255,.14);',
    '        overflow:hidden;display:inline-block;',
    '      ">',
    '        <span data-speedbar style="',
    '          display:block;height:100%;width:0%;border-radius:2px;',
    '          background:linear-gradient(90deg,#5fb8e8,#8fd8ff);transition:width .15s;',
    '        "></span>',
    '      </span>',
    ...
    '<div data-hint style="',
    '  position:absolute;right:16px;bottom:16px;padding:8px 14px;' + GLASS + MONO,
    '  font-size:11px;color:rgba(255,255,255,.62);pointer-events:none;',
    '">WASD move \u00b7 mouse look \u00b7 SPACE double-jump \u00b7 SHIFT sprint \u00b7 E discover</div>',

    // Discovery popup
    '<div data-popup style="',
    '  position:absolute;left:50%;bottom:84px;transform:translate(-50%,10px);max-width:380px;',
    '  padding:14px 18px;' + GLASS + 'border-color:rgba(143,216,255,.35);pointer-events:none;',
    ...
    // Top-center: settings + music
    '<div data-top-controls style="',
    '  position:absolute;top:16px;left:50%;transform:translateX(-50%);',
    '  display:flex;align-items:center;gap:10px;z-index:4;pointer-events:none;',
    '">',
    '  <button data-mobile-settings type="button" aria-label="Control settings" style="',
    '    display:none;width:44px;height:44px;' + BTN + 'border-radius:50%;color:rgba(255,255,255,.85);',
    '  ">',
    ...
    '    </svg>',
    '  </button>',
    '  <button data-music type="button" aria-label="Toggle music" aria-pressed="false" style="',
    '    display:flex;width:44px;height:44px;' + BTN + 'border-radius:50%;color:rgba(255,255,255,.55);',
    '  ">',
    '    <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" aria-hidden="true">',
    ...
    // Mobile: handedness panel
    '<div data-mobile-panel style="',
    '  display:none;position:absolute;top:68px;left:50%;transform:translateX(-50%);',
    '  padding:14px 16px;min-width:220px;' + GLASS + 'pointer-events:auto;z-index:4;touch-action:manipulation;',
    '">',
    ...
    '    <button data-handed-right type="button" style="',
    '      flex:1;padding:10px 8px;' + BTN + 'border-radius:10px;font-size:12px;',
    '      background:rgba(143,216,255,.22);border-color:rgba(143,216,255,.45);',
    '    ">RIGHT</button>',
    '    <button data-handed-left type="button" style="',
    '      flex:1;padding:10px 8px;' + BTN + 'border-radius:10px;font-size:12px;',
    '    ">LEFT</button>',
    '  </div>',
    ...
    // Mobile: virtual stick + action buttons
    '<div data-mobile style="display:none;position:absolute;inset:0;pointer-events:none;z-index:3;">',
    '  <div data-stick-zone style="',
    '    position:absolute;bottom:28px;left:28px;width:148px;height:148px;',
    '    pointer-events:auto;touch-action:none;',
    '  ">',
    '    <div data-stick-base style="',
    '      position:absolute;left:50%;top:50%;width:118px;height:118px;margin:-59px 0 0 -59px;',
    '      border-radius:50%;' + GLASS + 'box-shadow:inset 0 0 0 1px rgba(255,255,255,.06);',
    '    ">',
    '      <div data-stick-knob style="',
    '        position:absolute;left:50%;top:50%;width:52px;height:52px;margin:-26px 0 0 -26px;',
    '        border-radius:50%;background:rgba(143,216,255,.55);',
    ...
    '    position:absolute;bottom:36px;right:28px;display:flex;flex-direction:column;',
    ...
    '    <button data-btn-jump type="button" style="',
    '      width:86px;height:86px;border-radius:50%;' + BTN + 'font-size:12px;',
    '      background:rgba(143,216,255,.22);border-color:rgba(143,216,255,.4);',
    '    ">JUMP</button>',
    '  </div>',
    '</div>',

    // Click-to-start overlay
    '<div data-start style="',
    '  position:absolute;inset:0;display:none;align-items:center;justify-content:center;',
    '  background:rgba(5,9,16,.42);cursor:pointer;',
    '">',
    ...
    '  background:#0a0e14;transition:opacity .6s;z-index:5;',
    '">',
    '  <div style="text-align:center;">',
    '    <div style="font-size:22px;font-weight:700;letter-spacing:.42em;color:#fff;margin-bottom:18px;">INFINITE</div>',
    '    <div style="',
    '      width:220px;height:3px;border-radius:2px;background:rgba(255,255,255,.12);',
    '      margin:0 auto 12px;overflow:hidden;',
    '    ">',
    '      <div data-loadbar style="height:100%;width:0%;background:#8fd8ff;border-radius:2px;transition:width .2s;"></div>',
    '    </div>',
    '    <div data-loadtext style="' + MONO + 'font-size:11px;color:rgba(255,255,255,.55);">compiling shaders \u00b7 generating terrain</div>',
    '    <div style="',
    '      ' + MONO + 'font-size:10px;color:rgba(255,255,255,.38);',
    '      margin-top:22px;max-width:280px;line-height:1.5;letter-spacing:.02em;',
    '    ">',
    '      Music: \u201cA Drifting Lens\u201d by Amos Roddy',
    '      <br>',
    '      Used for ambient background only',
    '    </div>',
    '  </div>',
    '</div>'
  ].join('');
